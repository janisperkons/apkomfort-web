...
const styles = StyleSheet.create({
  ...
  approveNote: { marginTop: 16, padding: 10, backgroundColor: '#F7F3E8', borderRadius: 4, fontSize: 9.5, color: '#5B584D', lineHeight: 1.4 },
  approveNoteBold: { fontFamily: 'Noto Sans', fontWeight: 'bold', color: '#26251F' },
  terms: { marginTop: 26, fontSize: 8, color: '#8A8578', lineHeight: 1.6 },
  termsTitle: { fontSize: 8, color: '#8A8578', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 4, fontFamily: 'Noto Sans', fontWeight: 'bold' },
...
const TERMS = [
  'Piedāvājums ir spēkā līdz norādītajam derīguma termiņam; pēc tā beigām cenas var tikt pārskatītas.',
  'Cenā iekļauti norādītie darbi un materiāli; papildu darbi tiek saskaņoti un aprēķināti atsevišķi.',
  'Ja starp piedāvājuma sagatavošanu un darba izpildi būtiski mainās materiālu cenas, par to informēsim pirms darba sākuma.',
  'Klients nodrošina piekļuvi objektam saskaņotajā laikā; ja tas nav iespējams, lūdzam informēt vismaz 24 stundas iepriekš.',
  'Šis piedāvājums nav rēķins un nerada maksāšanas pienākumu, kamēr tas nav apstiprināts.',
  'Apstiprinot piedāvājumu, tiek saskaņots arī darba sākuma datums — rēķins tiek izrakstīts tikai pēc tam.',
]
...
export function QuoteDocument({ quote, items, canLogin }) {
  ...
  return e(Document, null,
    e(Page, { size: 'A4', style: styles.page },
      ...
      e(Text, { style: styles.brandSub }, `${COMPANY.legalName} · Reģ. Nr. ${COMPANY.regNr} · ${COMPANY.city} · ${COMPANY.phone}`),
      ...
      e(View, { style: styles.metaRow },
        e(View, { style: styles.metaBlock },
          ...
          quote.target_start_date && e(View, null,
            e(Text, { style: styles.label }, 'Plānotais sākums'),
            e(Text, { style: styles.value }, `${dateFmt(quote.target_start_date)} · ${quote.duration_days} ${quote.duration_days === 1 ? 'diena' : 'dienas'}`),
          ),
      ...
      quote.status === 'sent' && e(View, { style: styles.approveNote },
        canLogin
          ? e(Text, null, e(Text, { style: styles.approveNoteBold }, 'Lai apstiprinātu šo piedāvājumu, '), 'lūdzu ielogojieties savā kontā vietnē apkomforts.com un apstipriniet to tur.')
          : e(Text, null, e(Text, { style: styles.approveNoteBold }, 'Lai apstiprinātu šo piedāvājumu, '), 'atbildiet uz e-pastu vai piezvaniet — vienosimies par darba sākuma datumu.'),
      ),

      e(View, { style: styles.terms },
        e(Text, { style: styles.termsTitle }, 'Vispārīgie noteikumi'),
        ...TERMS.map((t, i) => e(Text, { key: i, style: { marginBottom: 2 } }, `${i + 1}. ${t}`)),
      ),

      e(Text, { style: styles.footer }, `${COMPANY.legalName} · Reģ. Nr. ${COMPANY.regNr} · ${COMPANY.phone} · Šis ir cenas piedāvājums, nevis rēķins.`),
    ),
  )
}

export async function renderQuotePdf({ quote, items, canLogin }) {
  return renderToBuffer(e(QuoteDocument, { quote, items, canLogin }))
}
