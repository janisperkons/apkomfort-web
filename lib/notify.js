import { sendMail, wrapEmailHtml, escapeHtml } from './mailer'
...
export async function notifyNewCustomerSignup(sb, { fullName, phone, email, isCompany, customerId }) {
  try {
    ...
    if (process.env.MAIL_USER) {
      await sendMail({
        ...
        html: wrapEmailHtml(`
          ...
          <p><b>${escapeHtml(fullName) || '—'}</b><br>${escapeHtml(phone) || '—'}<br>${escapeHtml(email) || '—'}</p>
