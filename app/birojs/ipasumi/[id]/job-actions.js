...
export default function JobActions({ job }) {
  ...
  if (mode !== 'complete') {
    return (
      <div style={{display:'flex',gap:8,marginTop:10}}>
        ...
        <button type="button" className="btn ghost" style={{fontSize:12.5,padding:'7px 12px'}} disabled={busy} onClick={()=>setMode('complete')}>Pabeigt darbu</button>
  ...
  return (
    <form onSubmit={complete} style={{marginTop:12,paddingTop:12,borderTop:'1px solid #EFEADC'}}>
      <label>Paveiktais darbs</label>
      <textarea value={workDone} onChange={e=>setWorkDone(e.target.value)} required />
      ...
      <div style={{display:'flex',gap:10,marginTop:12}}>
        <button className="btn" disabled={busy}>{busy ? 'Saglabā…' : 'Pabeigt darbu'}</button>
