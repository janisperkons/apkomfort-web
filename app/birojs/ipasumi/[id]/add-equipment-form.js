...
export default function AddEquipmentForm({ propertyId }) {
  ...
  async function submit(e) {
    ...
    const { error } = await supabaseBrowser().from('equipment').insert({
      property_id: propertyId, kind, manufacturer: manufacturer.trim() || null,
      model: model.trim() || null, installed_year: installedYear ? Number(installedYear) : null,
    })
    if (error) { setErr('Neizdevās saglabāt.'); setBusy(false); return }
    setManufacturer(''); setModel(''); setInstalledYear(''); setBusy(false); setOpen(false); router.refresh()
  }
